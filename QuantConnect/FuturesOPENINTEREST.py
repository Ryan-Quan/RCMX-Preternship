class FuturesOpenInterest(QCAlgorithm):
    ...
    def OnData(self, slice):
        ...
        # Looping over each available futures chain from slice.FutureChains data
        for chain in slice.FutureChains:
            
            #1. Filter to choose popular contracts with OpenInterest > 1000
            print(chain)
            self.popularContracts = [contract for contract in chain.Value if contract.Volume > 1000]
